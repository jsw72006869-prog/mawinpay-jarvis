      ...
      try {
        ...
        if (ssAction === 'get_products') {
          ...
          if (ssAction === 'query_order_status') {
            // SMARTSTORE-ORDERS-FIX.2B: dashboardSnapshot + detailSync 레이어 표시
            const snap = (data as any).dashboardSnapshot;
            const sync = (data as any).detailSync;
            const rangeNote = snap?.isRangeLimited ? `\n⚠ 조회 범위: 배송중/배송완료 30일, 구매확정 90일 (결제일 기준)` : '';
            const syncLabel = sync?.detailStatus === 'matched' ? '상세 동기화: 완료' : sync?.detailStatus === 'partial' ? '상세 동기화: 부분' : '상세 동기화: 필요';
            const orderableCount = sync ? (sync.newOrdersWithId + sync.pendingShippingWithId) : preT;
            resultMsg = `[PKG] **전체 주문/발주 현황** ${srcLabel}\n\n신규주문: ${nO}건\n배송준비: ${pS}건\n배송 전 처리 대상 전체: ${preT}건\n배송중: ${ship}건\n배송완료: ${dlvd}건\n구매확정: ${pConf}건\n\n현황: OBSERVE 조회 완료\n${syncLabel}\n발주 가능 대상: ${orderableCount}건 (ProductOrderId 확보)${rangeNote}`;
