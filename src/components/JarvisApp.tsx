...
응답은 한국어로, 실제 바이럴에 바로 쓸 수 있는 콘텐츠만 작성하세요.${params.researchPrefix || ''}`;
      ...
      try {
        ...
        if (ssAction === 'get_products') {
          ...
          if (ssAction === 'query_order_status') {
            ...
            doneText = `신규주문 ${nO}건, ${shipTts}입니다, 선생님.`;
  ...
  const handleTextSubmit = useCallback(async (text: string) => {
    ...
    if (outreachCollectFinal || outreachCollectMatchGeneric) {
      ...
      try {
        ...
        if (data.success && data.candidates && data.candidates.length > 0) {
          ...
          if (requireEmail && shortfall > 0) {
            // 조건 미충족: 부족 보고
            reportMsg = `**${keyword} 후보 수집 완료** (YouTube 공식 카테고리 전체 탐색, 이메일 조건 적용)\n\n` +
              `| 항목 | 수치 |\n|------|------|\n` +
              `| 요청 인원 | ${count}명 |\n` +
              `| 공개 이메일 확인 | ${finalCandidates.length}명 |\n` +
              `| 부족 | ${shortfall}명 |\n` +
              `| 이메일 없음 (제외) | ${excludedNoEmail.noEmail || 0}명 |\n` +
              `| 문의 링크만 (제외) | ${excludedNoEmail.contactLinkOnly || 0}명 |\n` +
              `| 적합도 60점↑ | ${highFit}명 |\n` +
              `| 탐색 세그먼트 | ${searchedSegments.join(', ')} |\n\n` +
            ...
            // 이메일 조건 없음
            reportMsg = `**공동구매 후보 ${finalCandidates.length}명 수집 완료** (YouTube 공식 카테고리 전체 탐색)\n\n` +
              `| 항목 | 수치 |\n|------|------|\n` +
              `| 적합도 60점↑ | ${highFit}명 |\n` +
          ...
          // ── 수집 완료 TTS 브리핑 ──
          const voiceReport = requireEmail
          ...
          // ── 자연스러운 후속 대화 ──
          const followUpMsg = contactable > 0
...
`;
  ...
  const handleActivate = useCallback(async () => {
    ...
    if (s === 'idle') {
      ...
      // 시그니처 응답: GPT 대기 없이 즉시 재생 (반복 방지)
      let sigResponse = SIGNATURE_RESPONSES[Math.floor(Math.random() * SIGNATURE_RESPONSES.length)];
      // 이전 응답과 동일하면 다음 것 선택
      if (sigResponse === lastAssistantMsgRef.current) {
        const idx = SIGNATURE_RESPONSES.indexOf(sigResponse);
        sigResponse = SIGNATURE_RESPONSES[(idx + 1) % SIGNATURE_RESPONSES.length];
      }
      setState('speaking');
      addMessage('jarvis', sigResponse);
      lastAssistantMsgRef.current = sigResponse;
      startSpeakingLevel();
      await new Promise<void>(resolve => {
  ...
  return (
    ...
    >
      ...
      <PredictiveActionPanel
        ...
        visible={predictedActions.length > 0 && !missionWorkspaceOpen && !outreachVisible && activeScene !== 'approval_gate' && activeScene !== 'smartstore_brief' && activeScene !== 'keyword_radar' && !copyFocusMode}
      ...
      {/* ── ActionCard (UI-CENTER-B.1: 중앙 모달로 변경) ── */}
      <div style={{
        position: 'fixed',
        top: '50%',
        // OUTREACH 패널이 열려 있으면 우측 420px를 피해 좌측으로 이동
        left: outreachVisible ? 'calc(50% - 210px)' : '50%',
        transform: outreachVisible ? 'translate(-50%, -50%)' : 'translate(-50%, -50%)',
        width: outreachVisible ? 'min(460px, calc(100vw - 440px))' : 'min(520px, 92vw)',
        maxHeight: '82vh',
        overflowY: 'auto',
        overflowX: 'hidden',
        zIndex: 60,
