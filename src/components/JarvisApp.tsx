  ...
  const executePendingActionFromChat = useCallback(async (decision: 'approve' | 'cancel') => {
    ...
    if (action.actionType === 'PURCHASE_ORDER_EMAIL_SEND') {
      const result = await postCloudTask('purchase-order-email-send-approved', {
        scope: 'pre_ship',
        groupIds: action.summary.groupIds || [],
        approvalConfirmed: true,
      ...
      speakJarvisSummary({ text: '발주서 이메일 전송 요청은 dryRun으로 처리했습니다. 실제 발송은 없습니다.', intent: 'purchase_order_email_send_dryrun', actionType: action.actionType });
      setState('idle');
      return;
    }
