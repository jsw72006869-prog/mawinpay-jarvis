...
export function composeJarvisBriefing(snapshot: JarvisSituationSnapshot, nextActions: JarvisNextAction[]): JarvisComposedResponse {
  ...
  const screenText = [
    ...
    `발주서 그룹은 ${snapshot.purchaseOrders.groupCount || 0}개이고, 이메일 초안이 가능한 그룹은 ${snapshot.purchaseOrders.readyEmailGroupCount || 0}곳입니다.`,
    ...
    (snapshot.outreach.remainingContactableCount || 0) > 0
      ? `Outreach는 목표까지 ${snapshot.outreach.remainingContactableCount}명이 더 필요합니다. 목표 미달이면 완료로 처리하지 않습니다.`
      : 'Outreach는 현재 저장된 요약 기준에서 긴급 미달 신호가 없습니다.',
    snapshot.risks.length ? `\n주의할 점: ${snapshot.risks.join(', ')}` : '',
    '',
    '추천 순서:',
    ...(topPriority.length ? topPriority.map((action, index) => `${index + 1}. ${priorityLabel(action)} - ${action.reason}`) : ['1. 발주서 정리해줘 - 현재 발주 그룹을 먼저 확인합니다.']),
    '',
    firstDraft
  ...
  const voiceSummary = readyGroups.length > 0
    ...
    : '대표님, 오늘은 발주서 설정 보강이 우선입니다. 이메일과 택배사 규칙이 필요한 그룹을 먼저 정리하겠습니다.';
