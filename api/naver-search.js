...
async function fetchNaverBlogProfile(blogId) {
    ...
    try {
        ...
        if (!result.email) {
            ...
            if (descMatch) {
                ...
                if (emailInDesc)
                    result.email = emailInDesc[0];
            }
        }
        // 이웃수 추출 (네이버 블로그 이웃 카운트)
        const neighborMatch = html.match(/이웃[^\d]*(\d[\d,]+)/);
        if (neighborMatch)
            result.neighborCount = parseInt(neighborMatch[1].replace(/,/g, ''), 10);
        // 방문자수 추출
        const visitorMatch = html.match(/방문[^\d]*(\d[\d,]+)/);
        if (visitorMatch)
            result.dailyVisitors = parseInt(visitorMatch[1].replace(/,/g, ''), 10);
    }
    catch (e) {
        // 크롤링 실패 시 빈 결과 반환
    }
...
module.exports = async function handler(req, res) {
    ...
    const { keyword, source = 'blog', display = '100', sort = 'sim', fetchProfile = 'false', // 프로필 크롤링 여부 (느리므로 선택적)
     } = req.query;
    if (!keyword)
    ...
    try {
        ...
        for (let page = 0; page < maxPages; page++) {
            ...
            if (start > 1000)
                break;
            const response = await fetch(`${apiUrl}?query=${encodeURIComponent(String(keyword))}&display=100&start=${start}&sort=${sort}`, {
                headers: {
