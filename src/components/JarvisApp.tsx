...
import { buildJarvisBusinessBrainContext } from '../lib/conversation-os/businessBrain';
import { rememberJarvisEvent, rememberNamedSnapshot } from '../lib/conversation-os/businessMemory';
import { buildViralInsightFromYouTubeCollection } from '../lib/conversation-os/viralInsight';
import { inferIntentFromUserText, parseYouTubeCollectionIntent } from '../lib/conversation-os/intent';
...
import { buildDeterministicCreativeCards, cardsToCreativeStudioCopies, resolveCreativeContextGuard, shouldUseDeterministicCreativeCards } from '../lib/conversation-os/creativeContext';
import type { JarvisNextAction, YouTubeCollectionIntent } from '../lib/conversation-os/types';
...
function buildJarvisVoiceSummary(text: string, intent?: string, actionType?: string): string {
  ...
  if (/outreach|인플루언서|유튜버|후보|목표|follow.?up|팔로우업/.test(source)) {
    return '현재 확보된 후보와 남은 기준을 화면에서 나눠 보고하겠습니다.';
  }
...
function buildJarvisVoiceSummaryClean(text: string, intent?: string, actionType?: string): string {
  ...
  if (/gmail|Gmail|email|이메일|메일/.test(source)) {
    return 'Gmail은 초안과 테스트 결과를 먼저 확인한 뒤, 선택한 단건부터 진행할 수 있습니다.';
  }
  if (/telegram|Telegram|텔레그램/.test(source)) {
    return '기존 9시 텔레그램 브리핑을 활용합니다. 새 봇이나 새 스케줄러를 만들지 않습니다.';
  }
  if (/outreach|인플루언서|유튜버|후보|목표/.test(source)) {
    return '수집 결과는 목표, 확보 후보, 남은 기준으로 나눠 보여드리겠습니다.';
  }
...
export default function JarvisApp() {
  ...
  const addMessage = useCallback((role: 'user' | 'jarvis' | 'assistant', text: string, isCompletion = false) => {
    ...
    const id = `${Date.now()}-${messageSequenceRef.current}`;
    setMessages(prev => [...prev, { id, role: normalizedRole, text, timestamp: new Date(), isCompletion }].slice(-8));
  }, []);
  ...
  const handleJarvisContextEvent = useCallback(async (event: JarvisContextEvent) => {
    ...
    try {
      ...
      // Intent별 context 구성
      if (event.intent === 'candidate_enrich_preview' && event.payload) {
        const payload: any = event.payload as any;
        const candidate = payload?.candidate || payload?.viewModel?.raw || payload;
        const channelId = String(candidate?.channelId || candidate?.id || candidate?.candidateId || '').trim();
        const keyword = String(candidate?.matchedKeyword || candidate?.keyword || candidate?.source_keyword || outreachCollectionSummary?.keyword || '').trim();
        if (!channelId) {
          addMessage('jarvis', '대표님, 이 후보는 channelId가 없어 보강 분석을 실행할 수 없습니다. 원본 채널 URL이 있는 후보를 선택해 주세요.', true);
          setState('listening');
          return;
        }
        addMessage('jarvis', `${candidate?.name || candidate?.channelName || '선택 후보'} 보강 분석을 시작합니다. 저장이나 발송은 하지 않는 dryRun 분석입니다.`, true);
        const res = await fetch('/api/cloud-proxy', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            taskType: 'youtube-candidate-enrich-preview',
            params: {
              channelId,
              keyword,
              categoryKey: outreachCollectionSummary?.categoryKey || outreachCollectionSummary?.requestedVertical || 'unknown',
              dryRun: true,
            },
          }),
        });
        ...
        if (!data?.success) {
          addMessage('jarvis', `보강 분석이 완료되지 않았습니다. 상태: ${data?.errorCode || data?.stopReason || 'api_error'}`, true);
          setState('listening');
          return;
        }
        setOutreachCandidates(prev => prev.map((item: any) => {
          const id = String(item.channelId || item.id || item.candidateId || '');
          if (id !== channelId) return item;
          return {
            ...item,
            enrichment: data,
            recentVideos: data.recentVideos || [],
            productFitScore: Number(data.productFit?.score || item.productFitScore || 0),
            fitScore: Number(data.productFit?.score || item.fitScore || item.productFitScore || 0),
            viralFitScore: Number(data.outreachFit?.score || item.viralFitScore || 0),
            productFitReason: data.productFit?.reason || item.productFitReason,
            reasonShort: data.productFit?.reason || item.reasonShort,
            recentAverageViews: data.recentAverageViews || item.recentAverageViews,
            publicEmailMasked: data.outreachFit?.emailMasked || item.publicEmailMasked,
            maskedEmail: data.outreachFit?.emailMasked || item.maskedEmail,
            emailStatus: data.outreachFit?.emailStatus || item.emailStatus,
            contactable: data.outreachFit?.status === 'contactable' || item.contactable,
            riskFlags: data.riskFlags || item.riskFlags,
            topVideoTitle: data.recentVideos?.[0]?.title || item.topVideoTitle,
            topVideoViewCount: data.recentVideos?.[0]?.viewCount || item.topVideoViewCount,
          };
        }));
        addMessage(
          'jarvis',
          [
            '보강 분석 결과입니다.',
            `- 채널: ${data.channelName || candidate?.name || '확인 필요'}`,
            `- 구독자: ${Number(data.subscriberCount || 0).toLocaleString('ko-KR')}`,
            `- 최근 영상: ${Array.isArray(data.recentVideos) ? data.recentVideos.length : 0}건`,
            `- 상품 적합도: ${data.productFit?.score ?? '확인 필요'}`,
            `- Outreach 적합도: ${data.outreachFit?.score ?? '확인 필요'}`,
            Array.isArray(data.riskFlags) && data.riskFlags.length ? `- 확인 필요: ${data.riskFlags.slice(0, 4).join(', ')}` : '',
            '원문 이메일이나 개인정보는 화면에 표시하지 않았고, 저장/발송도 실행하지 않았습니다.',
          ].filter(Boolean).join('\n'),
          true,
        );
        speakJarvisSummary({ text: '선택 후보 보강 분석을 완료했습니다. 최근 영상과 적합도 근거를 화면에 반영했습니다.', intent: 'candidate_enrich_preview' });
        setState('listening');
        return;
      } else if (event.intent === 'candidate_selected' && event.payload) {
  ...
  }, [addMessage, speak, setState, outreachCollectionSummary, speakJarvisSummary]);
...
응답은 한국어로, 실제 바이럴에 바로 쓸 수 있는 콘텐츠만 작성하세요.${params.researchPrefix || ''}`;
    ...
    if (action?.type === 'morning_briefing') {
      ...
      try {
        ...
        if (outreach.emailSent > 0) {
          ...
          briefingDisplay += `- 발송 현황: 대기 중 (대표 승인 필요)\n`;
        }
        ...
        voiceBriefing += `실제 실행이 필요한 작업은 대표님 확인을 받은 뒤 진행합니다. 이상입니다.`;
  ...
  const handleOutreachGoalCollectCommand = useCallback(async (goal: NonNullable<ReturnType<typeof parseOutreachGoalCommand>>) => {
    ...
    speakJarvisSummary({
      text: `${verticalLabel} 인플루언서 목표 수집을 시작합니다. 목표 기준과 현재 확보 수를 나눠서 보고하겠습니다.`,
    ...
    try {
      ...
      const statusLine = completionStatus === 'complete'
        ...
        : `목표까지 ${remainingContactableCount}명이 더 필요합니다. 이어서 확보할 후보를 분리해 두겠습니다.`;
      ...
      speakJarvisSummary({ text: '인플루언서 수집 API 호출이 실패했습니다. 현재 상태와 원인을 화면에 남겼습니다.', intent: 'outreach_goal_api_failed' });
    } finally {
      setState('idle');
    }
  }, [addMessage, createLocalPendingAction, postCloudTask, speakJarvisSummary]);
  ...
  const handleOutreachGoalCollectCommandV2 = useCallback(async (goal: any) => {
    ...
    speakJarvisSummary({
      text: `${verticalLabel} 인플루언서 목표 수집을 시작합니다. 목표 기준과 확보 현황을 분리해서 보고하겠습니다.`,
      intent: 'outreach_goal_collect_command',
    });
  ...
  const handleYouTubeCollectionPreviewCommand = useCallback(async (goal: YouTubeCollectionIntent) => {
    const targetCount = Math.max(1, Number(goal.targetCount || 20));
    const label = goal.categoryLabel || goal.keyword || 'YouTube';
    const isVideoMode = goal.targetType === 'video' || goal.mode.includes('video');
    const countOnly = Boolean(goal.countOnly);
    const businessBrain = buildJarvisBusinessBrainContext({
      userText: goal.originalUserText || `${label} YouTube collection`,
      intent: 'youtube_collection_preview',
      previewOnly: true,
      currentGoal: isVideoMode
        ? `${label} related video and reaction pattern preview`
        : `${label} related channel and candidate pool preview`,
      nextDecision: 'review candidates, continue collection, or build content ideas',
    });
    ...
    addMessage(
      'jarvis',
      [
        `대표님, ${label} ${isVideoMode ? '영상' : '채널'} 컬렉션을 미리 확인하겠습니다.`,
        `- 기준: ${goal.mode}`,
        `- 목표 표시 수: ${targetCount}`,
        '- 저장/발송/외부 실행: 없음',
        '',
        '결과가 나오면 전체 검색, 중복 제거, 후보/영상 수를 분리해서 보여드리겠습니다.',
      ].join('\n'),
      true,
    );
    speakJarvisSummary({
      text: `${label} ${isVideoMode ? '영상' : '채널'} 컬렉션을 미리 확인하겠습니다. 저장이나 발송은 하지 않습니다.`,
      intent: 'youtube_collection_preview',
    });

    try {
      const data = await postCloudTask('outreach-quality-batch-run', {
        keyword: goal.keyword || goal.categoryLabel || label,
        product: goal.categoryLabel || goal.keyword || label,
        requestedVertical: goal.categoryKey || 'unknown',
        verticalLabel: goal.categoryLabel || goal.keyword || label,
        platform: 'youtube',
        source: 'youtube_collection_os',
        businessGoal: businessBrain.businessContext.currentGoal,
        mode: 'goal_collect',
        collectionMode: goal.mode,
        targetType: goal.targetType,
        categoryKey: goal.categoryKey,
        categoryLabel: goal.categoryLabel,
        targetContactableCount: targetCount,
        requestedCount: Math.min(Number(goal.maxVideos || goal.maxChannels || targetCount), isVideoMode ? 50 : 50),
        maxCandidates: Math.min(Number(goal.maxChannels || targetCount), 50),
        maxBatches: 1,
        maxPages: goal.maxPages || 2,
        requireQualified: true,
        requireContactable: false,
      ...
      const summary = { ...(data?.summary || {}), ...(data?.diagnostics || {}) };
      const videos = Array.isArray(data?.videos) ? data.videos : Array.isArray(summary?.videos) ? summary.videos : [];
      const channels = Array.isArray(data?.channels) ? data.channels : Array.isArray(summary?.channels) ? summary.channels : [];
      const candidates = Array.isArray(data?.candidates) ? data.candidates : Array.isArray(summary?.candidates) ? summary.candidates : [];
      const rawSearchResultCount = Number(summary.rawSearchResultCount ?? summary.searchedVideoCount ?? data?.rawSearchResultCount ?? videos.length ?? 0) || 0;
      const dedupedChannelCount = Number(summary.dedupedChannelCount ?? summary.uniqueChannelCount ?? data?.dedupedChannelCount ?? channels.length ?? 0) || 0;
      const displayedCandidateCount = Number(summary.displayedCandidateCount ?? candidates.length ?? videos.length ?? 0) || 0;
      const contactableCount = Number(summary.contactableCount ?? data?.contactableCount ?? 0) || 0;
      const publicEmailCount = Number(summary.publicEmailCount ?? data?.publicEmailCount ?? 0) || 0;
      const completionStatus = String(summary.completionStatus || data?.completionStatus || (countOnly ? 'count_only' : 'preview'));
      const stopReason = String(summary.stopReason || data?.stopReason || (countOnly ? 'countOnly' : 'dryRun'));
      const viralInsight = buildViralInsightFromYouTubeCollection({
        keyword: goal.keyword || label,
        categoryLabel: goal.categoryLabel,
        videos,
        channels,
        candidates,
      ...
      rememberNamedSnapshot('last_youtube_collection', {
        collectionMode: goal.mode,
        targetType: goal.targetType,
        categoryKey: goal.categoryKey,
        categoryLabel: goal.categoryLabel,
      ...
      rememberNamedSnapshot('last_viral_insight', viralInsight);
      rememberJarvisEvent({
        type: 'youtube_collection',
        source: 'api_result',
        importance: rawSearchResultCount > 0 ? 'high' : 'medium',
        summary: `${label} ${isVideoMode ? 'video' : 'channel'} preview: raw ${rawSearchResultCount}, channels ${dedupedChannelCount}, displayed ${displayedCandidateCount}, contactable ${contactableCount}, status ${completionStatus}`,
        payload: {
          collectionMode: goal.mode,
          targetType: goal.targetType,
          keyword: goal.keyword,
      ...
      rememberJarvisEvent({
        type: 'viral_content_insight',
        source: 'api_result',
        importance: viralInsight.titleHooks.length > 0 ? 'medium' : 'low',
        summary: viralInsight.headline,
        payload: {
          titleHooks: viralInsight.titleHooks,
          creatorBuckets: viralInsight.creatorBuckets,
          contentIdeas: viralInsight.contentIdeas,
        },
      });

      const subjectLabel = goal.keyword
        ? `"${goal.keyword}"`
        : goal.categoryLabel || goal.categoryKey || 'YouTube';
      addMessage(
        'jarvis',
        [
          `대표님, ${subjectLabel} ${isVideoMode ? '영상' : '채널'} 컬렉션 미리보기 결과입니다.`,
          '',
          `- 전체 검색 결과: ${rawSearchResultCount}건`,
          `- 중복 제거 채널: ${dedupedChannelCount}개`,
          `- 화면 표시 후보/자료: ${displayedCandidateCount}건`,
          `- 공개 이메일 후보: ${publicEmailCount}명`,
          `- 연락 가능 후보: ${contactableCount}명`,
          `- 상태: ${completionStatus}`,
          `- 중단/제한 사유: ${stopReason}`,
          '',
          '이 작업은 미리보기라 Google Sheets 저장, Gmail 발송, Telegram 전송을 하지 않았습니다.',
          '다음에는 더 넓게 이어서 수집하거나, 현재 후보를 먼저 검토할 수 있습니다.',
        ].join('\n'),
        true,
      );
      addMessage(
        'jarvis',
        [
          `Marketing insight for ${subjectLabel}`,
          `- ${viralInsight.headline}`,
          viralInsight.titleHooks.length
            ? `- Top title hints: ${viralInsight.titleHooks.slice(0, 3).join(' / ')}`
            : '- Top title hints: more real video data is needed.',
          `- Suggested next move: ${viralInsight.contentIdeas[0]}`,
          '- No Google Sheets save, Gmail send, or Telegram send was executed.',
        ].join('\n'),
        true,
      );
      speakJarvisSummary({
        text: `${subjectLabel} 컬렉션 미리보기가 끝났습니다. 전체 검색과 중복 제거 결과를 화면에 정리했습니다.`,
        intent: 'youtube_collection_preview_result',
      });
    } catch (error: any) {
      const errorCode = String(error?.message || error || 'YOUTUBE_COLLECTION_PREVIEW_FAILED');
      addMessage(
        'jarvis',
        `대표님, YouTube 컬렉션 미리보기가 완료되지 않았습니다.\n\n상태: ${errorCode}\n저장/발송/외부 실행은 하지 않았습니다.`,
        true,
      );
      speakJarvisSummary({
        text: 'YouTube 컬렉션 미리보기가 실패했습니다. 저장이나 발송은 하지 않았습니다.',
        intent: 'youtube_collection_preview_failed',
      });
    } finally {
      setState('idle');
    }
  ...
  const handleDeterministicCreativeCommand = useCallback(async (userText: string) => {
    const requestedCount = extractRequestedCount(userText) || 5;
    const count = Math.max(5, Math.min(8, requestedCount));
    const context = resolveCreativeContextGuard({ userText });
    const cards = buildDeterministicCreativeCards(context, count);
    const copies = cardsToCreativeStudioCopies(cards);
    const resultText = copies.map((copy: any, index: number) => [
      `=== 카드 ${index + 1} ===`,
      `제목: ${copy.headline}`,
      copy.body,
    ].join('\n')).join('\n\n');
    ...
    setCreativeStudioVisible(true);
    setCreativeStudioLoading(false);
    setCreativeStudioProduct(context.productKeyword);
    setCreativeStudioType('reels_script');
    setCreativeStudioCopies(copies as CopyCard[]);
    setCreativeStudioTrends(cards.length);
    setCreativeStudioRefs(Number(outreachCollectionSummary?.rawSearchResultCount || outreachCollectionSummary?.searchedVideoCount || 0) || 0);
    setCreativeStudioMetadata({
      strategy: 'creative_context_guard',
      context,
      sourceBasis: context.basis,
    } as any);
    setResultDeckVisible(true);
    setResultDeckContent(resultText);
    setResultDeckType('reels_script');
    setResultDeckProduct(context.productKeyword);
    setResultDeckItems(copies);
    setCopyFocusMode(true);
    rememberJarvisEvent({
      type: 'creative_generation',
      source: 'system',
      importance: 'medium',
      summary: `${context.productKeyword} reels scripts generated with context guard from ${context.source}`,
      payload: {
        productKeyword: context.productKeyword,
    ...
    addMessage(
      'jarvis',
      [
        `대표님, ${context.productKeyword} 기준으로 릴스 대본 ${cards.length}개를 Creative Studio에 열었습니다.`,
        `- 맥락 출처: ${context.basis}`,
        '- 카드마다 title, hook, angle, script, shot list, caption, CTA를 분리했습니다.',
        '- 낡은 상품 맥락이 섞이지 않도록 stale keyword guard를 적용했습니다.',
      ].join('\n'),
      true,
    );
    speakJarvisSummary({
      text: `${context.productKeyword} 릴스 대본 ${cards.length}개를 생성했습니다. 최근 수집 맥락을 기준으로 카드별 각도를 다르게 구성했습니다.`,
      intent: 'creative_context_guard_completed',
    });
    setState('idle');
  }, [
    addMessage,
  ...
  const handleTextSubmit = useCallback(async (text: string) => {
    ...
    const youtubeCollectionIntent = parseYouTubeCollectionIntent(normalizedCommand);
    ...
    const wantsCreativeRefresh = creativeStudioVisible && /(더\s*자극|자극적으로|더\s*세게|더\s*강하게|후킹\s*강하게|더\s*후킹)/i.test(normalizedCommand);
    if (shouldUseDeterministicCreativeCards(normalizedCommand) || wantsCreativeRefresh) {
      await handleDeterministicCreativeCommand(normalizedCommand);
      return;
    }
    ...
    if (routedCommand.handled && routedCommand.intent === 'outreach_goal_continue_command') {
      const previousGoal: YouTubeCollectionIntent | null = outreachCollectionSummary
        ? {
            intent: 'collect_youtube',
            mode: outreachCollectionSummary.collectionMode || 'category_channel_collect',
            targetType: outreachCollectionSummary.targetType || (String(outreachCollectionSummary.collectionMode || '').includes('video') ? 'video' : 'channel'),
            categoryKey: outreachCollectionSummary.categoryKey || outreachCollectionSummary.requestedVertical,
            categoryLabel: outreachCollectionSummary.categoryLabel || outreachCollectionSummary.verticalLabel,
            keyword: outreachCollectionSummary.keyword,
            targetCount: Number(outreachCollectionSummary.targetCount || outreachCollectionSummary.targetContactableCount || 20) || 20,
            maxVideos: Number(outreachCollectionSummary.maxVideos || outreachCollectionSummary.targetCount || 20) || 20,
            maxChannels: Number(outreachCollectionSummary.maxChannels || outreachCollectionSummary.targetCount || 20) || 20,
            maxPages: 2,
            dryRun: true,
            countOnly: false,
            requiresApproval: false,
            actualExecution: false,
            originalUserText: normalizedCommand,
          }
        : youtubeCollectionIntent;
      if (previousGoal) {
        await handleYouTubeCollectionPreviewCommand({ ...previousGoal, countOnly: false });
        return;
      }
      addMessage('jarvis', '대표님, 이어서 수집할 카테고리나 키워드가 아직 명확하지 않습니다. 예를 들어 “캠핑 유튜버 20명 이어서 수집” 또는 “매실 영상 더 찾아줘”처럼 말씀해 주세요.', true);
      speakJarvisSummary({ text: '이어서 수집할 카테고리나 키워드가 필요합니다.', intent: 'youtube_collection_continue_missing_context' });
      setState('idle');
      return;
    }
    ...
    if (deterministicIntent.intent === 'outreach_goal_collect_command') {
      if (youtubeCollectionIntent) {
        await handleYouTubeCollectionPreviewCommand(youtubeCollectionIntent);
        return;
      }
    ...
    if (wantsOutreachGoalQuestion) {
      ...
      speakJarvisSummary({ text: '목표 조건을 충족한 후보 수와 남은 후보 수를 분리해서 보고합니다.', intent: 'outreach_goal_question' });
      setState('idle');
      return;
    }
...
`;
  ...
  }, [addMessage, creativeStudioVisible, handleDeterministicCreativeCommand, handleYouTubeCollectionPreviewCommand, jarvisRespond, showConversationOsBriefing, speakJarvisSummary, stopSpeakingLevel]);
