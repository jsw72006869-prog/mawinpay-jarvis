...
응답은 한국어로, 실제 바이럴에 바로 쓸 수 있는 콘텐츠만 작성하세요.${params.researchPrefix || ''}`;
  ...
  const executePendingActionFromChat = useCallback(async (decision: 'approve' | 'cancel') => {
    ...
    setState('idle');
  }, [addMessage, createLocalPendingAction, downloadPurchaseOrderFiles, postCloudTask]);
  ...
  const handleTextSubmit = useCallback(async (text: string) => {
    ...
    const wantsPurchaseOrderPrivacyQuestion =
      ...
      || /(개인정보\s*포함\s*발주서).*(가능|뭐야|어떻게|왜)/.test(normalizedCommand);
    ...
    if (wantsPurchaseOrderCommandHelp) {
      addMessage('jarvis', '발주서 쪽은 이렇게 말하시면 됩니다.\n\n- 발주서 정리해줘\n- 마스킹 발주서 다운로드해줘\n- 개인정보 포함 발주서 다운로드해줘\n- 매실 발주서 이메일 초안 보여줘\n- 복숭아는 로젠택배로 저장해\n- 매실 발주처 이메일 저장할게\n\n개인정보 포함 다운로드와 실제 이메일 전송은 각각 별도 승인을 받은 뒤에만 진행합니다.', true);
      setState('idle');
      return;
    }
