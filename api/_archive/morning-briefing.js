 ...
 * 
 * 3가지 데이터 소스를 동시에 수집하여 통합 보고:
 * 1. SmartStore: 신규 주문, 배송 준비, 매출
 * 2. Google Sheets: 인플루언서 누적 현황
 * 3. Gmail: 답장/협업 관련 메일 요약 (프론트엔드에서 MCP로 처리)
 */
...
async function fetchOrders(days, statuses = ['PAYED']) {
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    try {
      ...
      console.warn(`[morning-briefing] 주문 조회 실패:`, err.message);
    }
  ...
  for (let i = 0; i < allProductOrderIds.length; i += 300) {
    ...
    try {
      ...
      console.warn(`[morning-briefing] 상세 조회 실패:`, err.message);
    }
  }
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    try {
      // 오늘 신규 주문
      const todayOrders = await fetchOrders(1, ['PAYED']);
      ...
      addLog('SMARTSTORE', 'success', `신규 주문 ${todayNormalized.length}건 수집 완료`);

      // 배송 준비 중
      addLog('SMARTSTORE_SHIP', 'start', '배송 준비 주문 조회 중...');
      const pendingShip = await fetchOrders(7, ['PAYED']);
      addLog('SMARTSTORE_SHIP', 'success', `배송 준비 ${pendingShip.length}건`);

      // 어제 매출 (비교용)
      addLog('SMARTSTORE_COMPARE', 'start', '어제 매출 데이터 조회 중...');
      const yesterdayOrders = await fetchOrders(2, ['PAYED']);
      // 어제 주문만 필터 (오늘 제외)
