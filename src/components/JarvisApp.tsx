...
응답은 한국어로, 실제 바이럴에 바로 쓸 수 있는 콘텐츠만 작성하세요.${params.researchPrefix || ''}`;
  ...
  return (
    ...
    >
      ...
      {/* ── Three.js 파티클 배경 ── */}
      <SparkleParticles state={state} audioLevel={textInputMode ? 0 : micLevel} speakingLevel={textInputMode ? 0 : speakingLevel} clapBurst={clapBurst} freqData={micFreqData ?? undefined} />
      ...
      {/* ── 대화 패널 (Phase Prod-B) ── */}
      <AnimatePresence>
        {messages.length > 0 && activeScene !== 'smartstore_brief' && (
          <ConversationPanel
            messages={messages}
            isTyping={isTyping}
            sttStatus={sttStatus}
      ...
      <PredictiveActionPanel
        ...
        visible={predictedActions.length > 0 && activeScene !== 'approval_gate' && activeScene !== 'smartstore_brief' && !copyFocusMode}
