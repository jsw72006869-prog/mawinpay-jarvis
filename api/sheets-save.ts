...
// ── 시트 존재 확인 및 헤더 생성 ──
async function ensureSheet(token: string, sheetName: string, headers: string[]) {
  ...
  const headerRes = await fetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/${encodeURIComponent(sheetRange(sheetName, 'A1'))}`,
    { headers: { Authorization: `Bearer ${token}` } }
  );
  ...
  if (!headerData.values || headerData.values.length === 0) {
    ...
    await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/${encodeURIComponent(sheetRange(sheetName, `A1:${colLetter}1`))}?valueInputOption=RAW`,
      {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
...
// ── 마지막 행 번호 가져오기 ──
async function getLastRow(token: string, sheetName: string): Promise<number> {
  const res = await fetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/${encodeURIComponent(sheetRange(sheetName, 'A:A'))}`,
    { headers: { Authorization: `Bearer ${token}` } }
  );
...
async function appendRows(token: string, sheetName: string, rows: any[][]) {
