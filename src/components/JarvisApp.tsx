      ...
      try {
        ...
        if (data.success && data.candidates && data.candidates.length > 0) {
          ...
          if (detectedVertical !== 'unknown') {
            ...
            reportMsg = `**${verticalLabel} 인플루언서 후보 수집 완료** (Target Fit Gate 적용)\n\n` +
              `| 항목 | 수치 |\n|------|------|\n` +
              `| 요청 분야 | ${verticalLabel} |\n` +
              `| 적합 (qualified) | ${targetFitStats.qualified}명 |\n` +
              `| 검토 (review) | ${targetFitStats.review}명 |\n` +
              `| 분야 부적합 (제외) | ${targetFitStats.excludedTarget}명 |\n` +
              `| 이메일 없음 (제외) | ${excludedNoEmail.noEmail || 0}명 |\n` +
              `| 적합+이메일 확인 | ${targetFitStats.qualifiedWithEmail}명 |\n` +
              `| 최종 수집 | ${finalCandidates.length}명 |\n\n` +
              `${verticalLabel} 분야 근거 키워드가 확인된 후보만 저장했습니다. ` +
              `분야 부적합 ${targetFitStats.excludedTarget}명은 자동 제외되었습니다.\n\n` +
          ...
          const voiceReport = detectedVertical !== 'unknown'
            ? `${verticalLabel} 인플루언서 후보 수집이 완료되었습니다. ${verticalLabel} 분야 적합 후보 ${targetFitStats.qualified}명, 이메일 확인 ${targetFitStats.qualifiedWithEmail}명입니다. 분야 부적합 ${targetFitStats.excludedTarget}명은 자동 제외했습니다. 구글 시트에 저장했습니다.`
            : requireEmail
