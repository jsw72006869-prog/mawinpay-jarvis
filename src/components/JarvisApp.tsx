      ...
      try {
        ...
        if (data.success && data.candidates && data.candidates.length > 0) {
          ...
          const voiceReport = detectedVertical !== 'unknown'
            ? `${verticalLabel} 인플루언서 후보 수집이 완료되었습니다. 전체 검색 ${countSummary.rawSearchResultCount ?? 0}건 중 ${verticalLabel} 분야 적합 후보 ${qualifiedCount}명, 검토 필요 ${reviewCount}명, 분야 부적합 ${excludedCount}명입니다. 공개 이메일이 있는 적합 후보는 ${qualifiedPublicEmailCount}명입니다. 실제 발송은 잠금 상태입니다.`
            : requireEmail
