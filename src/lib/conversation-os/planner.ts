...
export function planJarvisNextActions(snapshot: JarvisSituationSnapshot): JarvisNextAction[] {
  ...
  if ((snapshot.smartstore.confirmNeededCount || 0) > 0) {
    actions.push({
      ...
      command: '발주확인 해줘',
      priority: 'high',
      reason: `발주확인이 필요한 주문 ${snapshot.smartstore.confirmNeededCount}건이 있습니다.`,
  ...
  groups.filter(group => !group.emailConfigured).slice(0, 2).forEach(group => {
    actions.push({
      ...
      priority: 'medium',
      reason: '발주처 이메일이 있어야 Gmail 초안과 승인 발송 흐름으로 넘어갈 수 있습니다.',
  ...
  if (snapshot.telegram.dailyBriefActive) {
    actions.push({
      ...
      priority: 'low',
      reason: '실제 Telegram 전송 전에 actionId 승인 흐름만 안전하게 점검합니다.',
