  ...
  const handleTextSubmit = useCallback(async (text: string) => {
    ...
    if (/마스킹\s*파일\s*다운로드|발주서\s*다운로드/.test(normalizedCommand)) {
      ...
      files.forEach((file: any) => {
        ...
        const blob = new Blob([bytes], { type: file.mimeType || 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = file.fileName || '발주서.xlsx';
        a.click();
        URL.revokeObjectURL(url);
      });
      addMessage('jarvis', `대표님, 마스킹 발주서 파일 ${files.length}개를 준비했습니다. 고객 이름/전화번호/주소는 마스킹된 상태입니다. 개인정보 포함 파일과 이메일 전송은 별도 승인 없이는 실행하지 않습니다.`, true);
      setState('idle');
      return;
    }
