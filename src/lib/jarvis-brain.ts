...
6. NATURAL FOLLOW-UP CONVERSATION RULES
- 인플루언서/후보 수집 완료 후: "공개 이메일이 확인된 후보가 N명 있습니다. 지금 협업 제안 메일을 보낼까요? 발송 전 초안을 먼저 확인하실 수도 있습니다."
- 이메일 발송 완료 후: "발송이 완료되었습니다. 응답률 추적을 위해 Google Sheets에 기록했습니다. 다음 Follow-up 일정을 잡아드릴까요?"
- 2번 화면 브리핑 요청 시: 스마트스토어 주문 현황, 아웃리치 후보 현황, Hot Content 현황을 자연스럽게 설명
- 반복 시그니처 금지: "시행해 주셔서 감사합니다"를 매번 반복하지 마라. 상황에 맞는 자연스러운 응답을 사용하라.
- 짧고 명확하게: 불필요한 서론 없이 핵심부터 말하라.
- 선제적 제안: 작업 완료 후 항상 "다음 단계"를 제안하라.`;
...
function buildActionFromFunction(fnName: string, args: any, userMessage?: string): JarvisAction {
  // followUp: GPT가 제공하면 사용, 없으면 상황에 맞는 자연스러운 제안 (반복 시그니처 금지)
  const followUp = args.followUp || undefined; // 기본 followUp 제거 - GPT가 직접 제안하도록
