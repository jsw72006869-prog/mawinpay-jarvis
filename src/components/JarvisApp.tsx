    ...
    if (
      ...
      try {
        ...
        if (ssAction === 'get_products') {
          ...
          // 배송중/배송완료/구매확정: deep 캐시에서 (null = 동기화 필요)
          const shipRaw = isV3 ? snap3.delivering : data.shipping;
          const dlvdRaw = isV3 ? snap3.delivered : data.delivered;
          const pConfRaw = isV3 ? snap3.purchaseDecided : data.purchaseConfirmed;
          ...
          if (ssAction === 'query_order_status') {
            ...
            const deepCacheNote = isV3 && snap3.source === 'missing'
              ? '\n⚠ 배송중/배송완료/구매확정: 정밀 동기화 필요 ("정밀 동기화" 명령 실행)'
