...
export default function JarvisApp() {
  ...
  const speakJarvisSummary = useCallback((input: {
    ...
    const summary = buildJarvisVoiceSummary(input.text, input.intent, input.actionType);
    if (!summary) return;
    const now = Date.now();
...
응답은 한국어로, 실제 바이럴에 바로 쓸 수 있는 콘텐츠만 작성하세요.${params.researchPrefix || ''}`;
    ...
    if (action?.type === 'morning_briefing') {
      ...
      try {
        ...
        let briefingDisplay = `[LIST] **자비스 일일 커맨드 리포트 v3.0** — ${today}\n\n`;
  ...
  const requestPurchaseOrderEmailSendApproval = useCallback((groupIds: string[]) => {
    ...
    createLocalPendingAction({
      ...
      summary: {
        ...
        totalOrderQuantity: selectedDrafts.reduce((sum, draft) => sum + Number(draft.totalQuantity || 0), 0),
        dryRun: false,
      },
      nextPrompt: isBulk
    ...
    addMessage('jarvis', isBulk
      ...
      : `발주서 이메일 1건 발송 승인 요청을 만들었습니다. 수신처는 ${selectedDrafts[0].recipientMasked || '마스킹됨'}으로만 표시합니다.`, true);
    speakJarvisSummary({ text: '실제 Gmail 발송에는 승인이 필요합니다. 승인 전까지 발송하지 않습니다.', intent: 'purchase_order_email_send_approval' });
  }, [addMessage, purchaseOrderEmailDraftPreview.drafts, speakJarvisSummary]);
  ...
  const parseOutreachGoalCommand = useCallback((rawText: string) => {
    ...
    const isOutreach = /(인플루언서|유튜버|블로거|크리에이터)/.test(normalized) && /(수집|찾아|모아|모집|검색|미리보기|테스트|dryrun|드라이런)/i.test(normalized);
  ...
  const handleOutreachGoalCollectCommand = useCallback(async (goal: NonNullable<ReturnType<typeof parseOutreachGoalCommand>>) => {
    ...
    speakJarvisSummary({ text: '인플루언서 목표 수집을 시작합니다. 목표 인원에 도달하기 전에는 완료로 처리하지 않습니다.', intent: 'outreach_goal_collect_command' });
    ...
    if (completionStatus === 'complete') {
      ...
      speakJarvisSummary({ text: '목표 인원에 도달했습니다. 다음 행동을 선택해 주세요.', intent: 'outreach_goal_complete' });
      createLocalPendingAction({
      ...
      speakJarvisSummary({ text: '목표 인원에 아직 도달하지 못했습니다. 부족한 인원과 중단 사유를 화면에 보고했습니다.', intent: 'outreach_goal_partial' });
      createLocalPendingAction({
    ...
    setState('idle');
  }, [addMessage, createLocalPendingAction, parseOutreachGoalCommand, postCloudTask, speakJarvisSummary]);
